...
# ### Data Ingestion for 50d embeddings
...
os.getcwd()
os.chdir('C:\\Users\\R\\Desktop\\MSDS 422\\run-jump-start-rnn-sentiment-big-v002\\run-jump-start-rnn-sentiment-big-v002')
...
# ### Note: 
# Model 1 and 2 will look at the default RNN model built compared to one with a dropout layer. They will use embeddings of 50 dimensions and 10000 word vocab size. Models 2, 3 and 4 will also look at the effect of changing the learning rate. However, they will use 100 dimension embeddings with a 40,000 vocab size. Models 3 and 4 will look at 3 layered models with and without dropout layers. Model performance will be judged based off of accuracy.

# ## Model 1  (50d, Simple RNN, 1 layer)
...
print('Average Train accuracy:', np.mean(acc_train)) 
print('Average Test accuracy:', np.mean(acc_test))


# ## Model 2 (50d, 3 layer LSTM) 
...
# ## Load 100d embeddings for Models 3, 4 and 5
...
# ## Model 3 (100d, Simple RNN with dropout layer)
...
dropout = tf.layers.dropout(logits, rate=.1, seed = RANDOM_SEED)
...
# ## Model 4 (100d, 3 layer LSTM with dropout layer)
...
# ## Model 5 (100d, 6 layer RNN with dropout layer)
...
# ## Conclusion:
# 
# It is apparent that adding a dopout layer and increasing the number of layers has a positive effect on model performance. It was also apparent that increasing the learning rate to a certain extent also had a positive effect. The best performing model was Model 4 which was a 3 layered RNN with a dropout layer and a learning rate of .005. It average 100% training accuracy and 69% testing accuracy. However, in Model 5 the number of layers was changed from 3 to 6 and there did not seem to be any increase in performance.
